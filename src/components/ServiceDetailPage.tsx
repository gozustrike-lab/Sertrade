...
/* ═══════════════════════════════════════════════════
   FALLBACK DATA — matches ServicesPage.tsx exactly
   ═══════════════════════════════════════════════════ */
...
export default function ServiceDetailPage({ category, services, fallbackSlug }: ServiceDetailPageProps) {
  ...
  return (
    <div>
      ...
      {/* ════════════════════════════════════════════════════════════════
          BLOCK 1: PANORAMIC COVER — Full-width hero image + Logo + Title
          ════════════════════════════════════════════════════════════════ */}
      <section className="relative w-full">
          ...
          >
            ...
            <div className="relative z-10 flex flex-col items-center text-center px-4">
              ...
              {moduleData.categoryDescription && (
                <p
                  className="text-white/75 text-sm md:text-base max-w-2xl mx-auto mt-4 leading-relaxed text-center"
        ...
        {/* ════════════════════════════════════════════════════════════════
            BLOCK 2: CATEGORY BAR — Translucent blur strip + floating icon
            ════════════════════════════════════════════════════════════════ */}
        <ScrollReveal animation="fade-down" delay={0.15}>
          <div className="relative w-full bg-[#004691]/65 backdrop-blur-xl py-5 text-center z-20 mt-0 mb-0 flex flex-col items-center justify-center border-b border-white/10">
            ...
            <span
              className="text-white text-lg md:text-2xl font-black tracking-widest uppercase mt-2 drop-shadow-md z-10"
          ...
          <div className="max-w-6xl mx-auto">
            <ScrollReveal>
              <div className="text-center pt-12 pb-4 md:pt-16 md:pb-6">
                ...
                <h2 className="text-2xl sm:text-3xl font-black text-[#004691] tracking-tight">
                  Áreas de {moduleData.category}
                </h2>
              ...
              >
                {moduleData.cards.map((card, cardIdx) => {
                  ...
                  return (
                    ...
                    >
                      {/* Tall vertical image — Portrait + clickable overlay */}
                      ...
                      <div
                        className="space-y-1 mt-4 mb-8 text-center"
                        {...(cardId ? ve(cardId, 'service', 'description') : {})}
                      >
                        <h3 className="text-gray-900 text-base md:text-lg font-bold tracking-wide uppercase">
                          {card.title}
                        </h3>
            ...
            <div className="md:hidden flex items-center justify-center gap-3 pb-8">
              <button
                onClick={() => scrollTo(Math.max(0, activeDot - 1))}
                className="text-gray-400 active:text-[#004691] transition-colors"
                aria-label="Anterior"
              >
              ...
              <button
                onClick={() => scrollTo(Math.min(moduleData.cards.length - 1, activeDot + 1))}
                className="text-gray-400 active:text-[#004691] transition-colors"
      ...
      </section>{/* closes <section className="relative w-full"> — wrapper for blocks 1, 2, 3 + CTA */}
    </div>
  );
}
